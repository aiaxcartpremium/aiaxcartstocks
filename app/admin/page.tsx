...
export default function AdminPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!res.ok) return;               // auto-redirect to /login kung wala
  ...
  async function onMarkSold(s: Stock) {
    ...
    const { error } = await supabase.rpc('mark_sold', { p_stock_id: s.id, p_buyer_email: buyer, p_price: price });
    if (error) alert(error.message); else fetchRows();
  }
  ...
  return (
    <div className="max-w-6xl mx-auto py-6">
      <header className="flex items-center justify-between mb-6">
        <h1 className="text-3xl font-extrabold">{role === 'owner' ? 'Owner' : 'Admin'} • Stocks</h1>
        <LogoutButton />
      </header>
  ...
  {PRODUCT_CATEGORIES.map((c: Category) => (
    <option key={c} value={c}>
      {c.charAt(0).toUpperCase() + c.slice(1)}
        ...
        <button onClick={fetchRows} className="ml-auto border rounded px-3 py-1.5 text-sm hover:bg-neutral-100">Refresh</button>
      </div>
          ...
          <tbody>
            {rows.map(r => (
              <tr key={r.id} className="border-t">
                ...
                <td className="p-2">
                  {r.status !== 'sold' && (
                    <button onClick={()=>onMarkSold(r)} className="border rounded px-2 py-1 hover:bg-neutral-100">Mark as sold</button>
                  )}
                </td>
              </tr>
            ))}
