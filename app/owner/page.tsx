...
import { PRODUCT_CATEGORIES, type ProductCategory } from '../../lib/catalog';
...
export default function OwnerPage() {
  ...
  function exportCsv() {
    ...
    const lines = rows.map(r => [r.id,r.product,r.category,r.status,r.price ?? '',r.buyer_email ?? '',r.created_at]
      .map(x=>`"${String(x).replace(/"/g,'""')}"`).join(','));
    const csv = [header.join(','), ...lines].join('\n');
    const blob = new Blob([csv],{type:'text/csv'}); const a = document.createElement('a');
    a.href = URL.createObjectURL(blob); a.download = 'owner-stocks.csv'; a.click(); URL.revokeObjectURL(a.href);
  }
      ...
      <div className="overflow-x-auto border rounded">
        <table className="min-w-[720px] w-full text-sm">
          ...
          <tbody>
            ...
            {rows.length===0 && <tr><td className="p-4 text-center text-neutral-500" colSpan={7}>No rows</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
